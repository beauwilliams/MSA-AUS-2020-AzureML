...
export class Parser {
    ...
    parse(tokens, renderer) {
        ...
        this.inline = new InlineLexer(tokens.links);
    ...
    parseText() {
        ...
        while (this.peek().type === 'text')
            body += '\n' + this.next().text;
        return renderer.create("span", { _innerHTML: this.inline.output(body) });
    }
    tok() {
        const renderer = this.renderer;
        switch (this.token.type) {
            ...
            case 'heading':
                ...
                return renderer.create(this.options.Heading, { depth: this.token.depth, slug: slug },
                    renderer.create("span", { _innerHTML: this.inline.output(this.token.text) }));
            case 'code':
                return renderer.create(this.options.Code, { lang: this.token.lang || '' }, this.token.text);
            case 'table': {
                const header = this.token.header.map((cell, i) => renderer.create(this.options.TableHeaderCell, { align: this.token.align[i] },
                    renderer.create("span", { _innerHTML: this.inline.output(cell) })));
                const body = this.token.cells.map(row => renderer.create(this.options.TableRow, null, row.map((cell, j) => renderer.create(this.options.TableCell, { align: this.token.align[j] },
                    renderer.create("span", { _innerHTML: this.inline.output(cell) })))));
            ...
            case 'paragraph':
                return renderer.create(this.options.Paragraph, null,
                    renderer.create("span", { _innerHTML: this.inline.output(this.token.text) }));
            case 'text':
